...
def search():
    if request.method == "POST":
        ...
        if search_type == "location":
            ...
            return redirect(url_for('result', genre=genre, area=area, range=range))
...
@app.route("/<string:genre>/<double:lat>/<double:lng>/<string:range>/result")
def result(genre, lat, lng, range):
    # ページングの処理
    page = request.args.get(get_page_parameter(), type=int, default=1)
    results = search_hotpepper_datum(genre, latitude, longitude, range)  # ここに実際の処理を追加する必要があります
    res = results[(page - 1) * 2: page * 2]
    pagination = Pagination(page=page, total=len(results), per_page=2, css_framework='bootstrap4')
    #redirect_url_with_page = url_for('result', page=page)
    return render_template("display.html", results=res, pagination=pagination)
