...
def result(page_id):
    ...
    if search_type == "location":
        range_value = int(request.args.get("search-range"))
        location = request.args.get("location")
        area = area_code(location)
        genre = request.args.get("search-genre")
        genre = genre_code(genre)
        results = search_hotpepper_area(genre, area, range_value)
    elif search_type == "current-location":
        range_value = int(request.args.get("search-range"))
        ...
        genre = request.args.get("search-genre")
        genre = genre_code(genre)
        results = search_hotpepper_datum(genre, latitude, longitude, range_value)
