...
def search_type_to_redirect(search_type):
    if search_type == "location":
        ...
        return redirect(url_for('result_area', genre=genre, area=area, range=range_value, _external=True))
        ...
        # ジャンルはコードに変更
        genre = genre_code(genre)
        return redirect(url_for('result_coordinates', genre=genre, lat=latitude, lng=longitude, range=range_value, _external=True))
...
# 検索結果ページ(緯度・経度)
#---------------------------------
@app.route("/<string:genre>/<float:lat>/<float:lng>/<string:range>/result_coordinates")
